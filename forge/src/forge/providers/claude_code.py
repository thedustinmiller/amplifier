...
"""
    ...
    async def _generate_commands(
        ...
        files = []
        tools = composition.get_tools()

        for tool in tools:
            command_file = claude_dir / "commands" / f"{tool.name}.md"
            content = self._format_command(tool)
    ...
    def _format_command(self, tool: Element) -> str:
        """Format tool as Claude Code command."""
        frontmatter = self._build_command_frontmatter(tool)

        instructions = tool.content or f"Execute {tool.name} tool."
        if tool.implementation and "instructions" in tool.implementation:
            instructions = tool.implementation["instructions"]
...
"""
    ...
    def _build_command_frontmatter(self, tool: Element) -> str:
        """Build command frontmatter."""
        lines = []

        if tool.metadata.description:
            description = tool.metadata.description.replace('"', '\\"')
            lines.append(f'description: "{description}"')
        ...
        if tool.implementation and "allowed_tools" in tool.implementation:
            allowed_tools = tool.implementation["allowed_tools"]
